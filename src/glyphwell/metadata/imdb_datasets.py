...
def _open_rows(path: Path) -> tuple[IO[str], list[str], Iterator[tuple[int, list[str]]]]:
    ...
    Returns the open handle (the caller must close it), the header, and a
    ``(row_number, values)`` iterator whose values are pre-checked against the header's
    column count.
...
def _basics_rows(path: Path) -> Iterator[TitleRow]:
    ...
    """
    handle, fieldnames, rows = _open_rows(path)
...
def _episode_links(path: Path) -> Iterator[EpisodeLink]:
...
def import_basics(
    conn: sqlite3.Connection, path: Path, *, batch_size: int = _DEFAULT_BATCH_SIZE
) -> int:
    ...
    return _run_in_batches(
        _basics_rows(path), batch_size=batch_size, conn=conn, write=repo.upsert_many
    )
...
def import_episodes(
    ...
    repo = TitlesRepository(conn)
    return _run_in_batches(
        _episode_links(path), batch_size=batch_size, conn=conn, write=repo.set_episode_links_many
    )
