...
Documents voisins : [docs/adr/](docs/adr/) pour le *pourquoi* des décisions structurantes
et les options écartées, [CHANGELOG.md](CHANGELOG.md) pour l'état livré et la surface
d'API publique. Ce fichier reste le point d'entrée.
...
2. **`run_files.last_sentence_index` est le point de reprise.** C'est la position, dans le
   flux de phrases du fichier, de la dernière phrase effectivement couverte par une fenêtre
   dont le résultat est committé. `last_sentence_id` conserve l'attribut `<s id>`
   correspondant, **pour la traçabilité uniquement** : cet attribut est un ordinal opaque
   (cf. §4), il ne peut donc pas servir à répondre « reprendre après ce point ».
...
| `run_files` | File de travail et **point de reprise** (`last_sentence_index`) par fichier. |
...
schéma et migrations SQLite (`db init`, `db status`, `db vacuum` fonctionnent), chargement
+ validation + hachage des manifestes YAML, calcul de sha256.

La CLI est **entièrement câblée** — les quatre groupes et toutes leurs options existent et
répondent à `--help` — mais seul le groupe `db` a des corps implémentés : les commandes de
`corpus`, `metadata` et `search` lèvent `NotImplementedError` (9 au total).

**Stubs typés** (`raise NotImplementedError`, signatures déjà complètes et vertes sous
mypy strict) — 72 points d'entrée répartis sur 17 modules :

| Module | À implémenter |
|---|---|
| [cli/corpus.py](src/glyphwell/cli/corpus.py), [cli/metadata.py](src/glyphwell/cli/metadata.py), [cli/search.py](src/glyphwell/cli/search.py) | corps des commandes (câblage déjà fait) |
| [manifest/prefilter.py](src/glyphwell/manifest/prefilter.py) | compilation et application du pré-filtre de motifs |
