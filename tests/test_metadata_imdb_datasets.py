...
def test_import_basics_populates_titles(catalog_db: sqlite3.Connection, tmp_path: Path) -> None:
    path = tmp_path / "title.basics.tsv"
    path.write_text(BASICS_TSV, encoding="utf-8")

    written = import_basics(catalog_db, path)

    assert written == 3
    found = TitlesRepository(catalog_db).get("tt0133093")
...
def test_import_episodes_links_after_basics(catalog_db: sqlite3.Connection, tmp_path: Path) -> None:
    ...
    import_basics(catalog_db, basics_path)
    written = import_episodes(catalog_db, episode_path)

    assert written == 1
    episode = TitlesRepository(catalog_db).get("tt0041039")
...
def test_import_basics_reports_progress_by_bytes(
    catalog_db: sqlite3.Connection, tmp_path: Path
) -> None:
    ...
    import_basics(catalog_db, path, progress=lambda current, total: calls.append((current, total)))
...
def test_import_episodes_reports_progress_by_bytes(
    catalog_db: sqlite3.Connection, tmp_path: Path
) -> None:
    basics_path = tmp_path / "title.basics.tsv"
    basics_path.write_text(BASICS_TSV, encoding="utf-8")
    ...
    calls: list[tuple[int, int]] = []
    import_episodes(
        catalog_db, episode_path, progress=lambda current, total: calls.append((current, total))
    )
...
def test_import_basics_is_idempotent(catalog_db: sqlite3.Connection, tmp_path: Path) -> None:
    """Re-running `import-imdb` on the same file must not duplicate rows."""
    path = tmp_path / "title.basics.tsv"
    path.write_text(BASICS_TSV, encoding="utf-8")

    import_basics(catalog_db, path)
    import_basics(catalog_db, path)

    assert TitlesRepository(catalog_db).count() == 3
...
def test_reimporting_basics_preserves_the_episode_link(
    ...
    import_basics(catalog_db, basics_path)
    import_episodes(catalog_db, episode_path)
    import_basics(catalog_db, basics_path)
...
def test_import_basics_batches_across_transactions(
    catalog_db: sqlite3.Connection, tmp_path: Path
) -> None:
    ...
    written = import_basics(catalog_db, path, batch_size=2)

    assert written == 5
    assert TitlesRepository(catalog_db).count() == 5
